...
describe 'implicit joins' do
  describe 'on belongs_to' do
    describe 'with single condition inline' do
      ...
      it 'should add correct join' do
        @params[:joins].should == %q(INNER JOIN "blogs" AS posts__blog ON "posts".blog_id = posts__blog.id)
      end

      it 'should query against condition on join table' do
        @params[:conditions].should == ['posts__blog.name = ?', 'Test Name']
      end
    end
    ...
    shared_examples_for 'multiple conditions on single join' do
      ...
      it 'should query against conditions on join table' do
        @params[:conditions].should == [%q((posts__blog.name = ?) AND (posts__blog.published = ?)), 'Test Name', true]
      end
    end
  ...
  describe 'on has_many' do
    before do
      @params = Blog.filter do
        having(:posts).with :permalink, 'test-post'
      end.proxy_options
    end

    it 'should add correct join' do
      @params[:joins].should == %q(INNER JOIN "posts" AS blogs__posts ON "blogs".id = blogs__posts.blog_id)
    end

    it 'should query against condition on join table' do
      @params[:conditions].should == [%q(blogs__posts.permalink = ?), 'test-post']
    end
  end
  ...
  describe 'with multiple joins' do
    ...
    it 'should query against both conditions' do
      @params[:conditions].should == [%q((blogs__posts.permalink = ?) AND (blogs__posts__comments.offensive = ?)), 'test-post', true] 
    end
  end
  ...
  describe 'with one having statement expressing multiple joins' do
    ...
    it 'should query against both conditions' do
      @params[:conditions].should == [%q(blogs__posts__comments.offensive = ?), true] 
    end
  end
  ...
  describe 'on has_one' do
    ...
    it 'should add correct join' do
      @params[:joins].should == %q(INNER JOIN "photos" AS posts__photo ON "posts".id = posts__photo.post_id)
    end

    it 'should query against condition on join table' do
      @params[:conditions].should == ['posts__photo.format = ?', 'jpg']
    end
  end
  ...
  describe 'with nested joins' do
    ...
    it 'should query against condition on join table' do
      @params[:conditions].should == ['blogs__posts__photo.format = ?', 'png']
    end
  end
  ...
  describe 'with has and belongs to many joins' do
    ...
    it 'should add correct join' do
      @params[:joins].should == %q(INNER JOIN "posts_tags" AS __posts__tags ON "posts".id = __posts__tags.post_id ) +
  ...
  describe 'with negative conditions' do
    before do
      @params = Comment.filter do
        with :offensive, false
      end.proxy_options
    end

    it 'should create the correct condition' do
      @params[:conditions].should == [%q("comments".offensive = ?), false]
    end
  end
  ...
  describe 'with nil conditions' do
    ...
    it 'should create the correct IS NULL condition' do
      @params[:conditions].should == [%q(("comments".contents IS NULL) AND ("comments".offensive = ?)), true]
    end
  end
  ...
  describe 'with negated nil conditions' do
    ...
    it 'should create the correct IS NOT NULL condition' do
      @params[:conditions].should == [%q(("comments".contents IS NOT NULL) AND ("comments".offensive = ?)), true]
    end
  end
  ...
  describe 'passing the join type to having' do
    ...
    it 'should create the correct condition' do
      @params[:conditions].should == [%q(blogs__posts.permalink = ?), 'ack']
    end

    it 'should create the correct join' do
      @params[:joins].should == %q(LEFT OUTER JOIN "posts" AS blogs__posts ON "blogs".id = blogs__posts.blog_id)
    end
  end
  ...
  describe 'passing the join type to having with multiple joins' do
    ...
    it 'should create the correct join' do
      @params[:joins].should == %q(LEFT OUTER JOIN "posts" AS blogs__posts ON "blogs".id = blogs__posts.blog_id LEFT OUTER JOIN "comments" AS blogs__posts__comments ON blogs__posts.id = blogs__posts__comments.post_id)
    end
  end
  ...
  describe 'on polymorphic associations' do
    ...
    it 'should create the correct condition' do
      @params[:conditions].should == [%q(posts__reviews.stars_count = ?), 3]
    end

    it 'should create the correct join' do
      @params[:joins].should == %q(INNER JOIN "reviews" AS posts__reviews ON "posts".id = posts__reviews.reviewable_id AND (posts__reviews.reviewable_type = 'Post'))
    end
  end
  ...
  describe 'on has_many_through associations' do
    ...
    it 'should create the correct join' do
      @params[:joins].should == %q(INNER JOIN "posts" AS blogs__posts ON "blogs".id = blogs__posts.blog_id INNER JOIN "comments" AS blogs__posts__comments ON blogs__posts.id = blogs__posts__comments.post_id)
    end
  end
  ...
  describe 'on has_one_through associations' do
    ...
    it 'should create the correct condition' do
      @params[:conditions].should == [%q(posts__author__user.first_name = ?), 'Joe']
    end

    it 'should create the correct join' do
      @params[:joins].should == %q(INNER JOIN "authors" AS posts__author ON "posts".id = posts__author.post_id INNER JOIN "users" AS posts__author__user ON posts__author.user_id = posts__author__user.id)
    end
  end
end
